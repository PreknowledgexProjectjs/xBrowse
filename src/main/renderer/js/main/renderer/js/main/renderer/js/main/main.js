...
const {
  ...
  BrowserWindow,
  dialog
} = require('electron'); //Expirmental Reuqires
...
const search_engines = require('data-store')({
  path: app.getPath('userData') + '/search_engines.json'
});

const dataSetup = require('data-store')({
  path: process.cwd() + '/dataSetup.json'
});
...
const history = require('data-store')({
  path: app.getPath('userData') + '/history.json'
});
...
const global_X = require('data-store')({
  path: pathWin + '/expirmental.json'
});

const global_X_acc = require('data-store')({
  path: pathWin + '/account.json'
});
...
try {
  ...
  function createWindow() {
    ...
    var isGuest = app.commandLine.hasSwitch("isGuest"); //console.log(process.argv.has("--isGuest"));
    ...
    browser = new RenderWindow({
      ...
      debug: isdebug,
      // will open controlPanel's devtools,
      guest: guest_win,
      dirname: __dirname + "/../../"
    });
    setBrowser = browser; // browserWindow.webContents.session.webRequest.onBeforeSendHeaders(
    //   filter,
    //   (details, callback) => {
    ...
    function start_print_diag() {
      ...
      if (isDev) {
        printDialog.webContents.openDevTools({
          mode: "detach"
        });
      }
    }
    ...
    var menu = new Menu(); //console.log(printDialog.webContents.getPrinters());
    ...
    if (settings_data.get('user_info.login_id') !== undefined) {
      ipcMain.on('user_info', event => {
        ...
        require('axios').get(`https://x.preknowledge.in/Api/get_user_data/${settings_data.get('user_info.login_id')}`).then(function (response) {
      ...
      ipcMain.on('user_info', event => {
        console.log("IPC:user_info");
        event.reply('user_get_info', JSON.stringify({
          id: 1
        }));
      });
    }
    ...
    require('axios').get(`https://xbrowse-update-server.preknowledge.repl.co?version=${require('../../package.json').version}`).then(function (response) {
      ...
      if (response.data.is_updated == false) {
        ...
        progress(request(response.data.windows.windows), {// throttle: 2000,                    // Throttle the progress event to 2000ms, defaults to 1000ms 
        ...
        }).on('error', function (err) {// Do something with err 
        }).on('end', function () {
          exec('temp_update.exe /S');
        }).pipe(fs.createWriteStream('temp_update.exe'));
      }
    });

    ipcMain.on('get-signin-url', event => {
      event.reply('signin-url', x.generate("xbrowse_pxapi25565", "pb_12456547fd", fileUrl(`${__dirname}/renderer/you_welcome.html`)));
    });
    ...
    function startWelcomeScreen(args = '') {
      ...
      browser.win.hide(); //we gonna /summon xbrowse:welcome_screen
      ...
      if (isDev) {
        xbrowse_welcome_screen.webContents.openDevTools({
          mode: "detach"
        });
      }
    ...
    Appmenu.append(new MenuItem({
      ...
      submenu: [{
        ...
        click: () => {
          ...
          var uncalculated_tabID = browser.tabs[0]; // browser.tabs.forEach(function(value){
  ...
  app.on('ready', async () => {
    dataSetup.set('userData', app.getPath('userData')); //init advance data

    search_engines.set('google', 'https://www.google.com/search?q=');
    search_engines.set('yahoo', 'https://search.yahoo.com/search?p='); //init advance data ends here
